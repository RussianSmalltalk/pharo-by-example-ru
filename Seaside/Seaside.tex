...
Вторая, с изображением лупы, открывает объектный инспектор текущего активного экземпляра.
Третья ссылка с цветными кругами показывает \ind{CSS} стиль компонента.
В правой части, гиперссылки \link{R} и \link{S} позволяют просмотреть нам готовую веб-страницу и в виде HTML-кода.
Поэксперементируйте с этими ссылками.
Обратие внимание, что гиперссылки \link{++} и \link{- -} остаются активными и в HTML-коде.
...
откройте инспектор текущего объекта Smalltalk counter и выполните \ct{self halt}.
Загрузка веб-страницы будет приостановленна.
Переключитесь на Seaside образ.
И в нем увидите отладочное окно (\figref{haltingCounter}) показываеющее
обработку сообщения \ct{halt} экземпляром \ct{WACounter}.
Проверив выполнение в отладчике, далее нажмите \button{Proceed}.
Вернувшись обратно в браузер, земетьте, что приложение продолжило свою работу.}
...
\url{http://localhost:8080/seaside/examples/multicounter}.
Вы увидете приложение, которое построено на наскольких независимых экземплярах класса Counter.
Попробуйте увеличивать и уменьшать значение счетчиков.
Убедитесь, что счетчики работают корректно даже после использования кнопки <<назад>>.
Переключите Halos, чтобы увидеть как построено приложение из вложенных компонентов.
Воспользуйтесь классовым браузером Seaside,
чтобы просмотреть имплементацию \ct{WAMultiCounter}.
Вы увидете три метода на стороне класса (\ct{canBeRoot},
\ct{description}, и \ct{initialize}) и три на стороне экземпляра (\ct{children},
\ct{initialize}, и \ct{renderContentOn:}).
Заметье, что приложение является простым компонентом,
которое готово быть корнем для вложенных компонентов;
готовность быть корнем определяется методом \ct{canBeRoot},
...
Попробуйте выполнить нежеследующие изменения конфигурации.
...
и \emph{deployment} (развернутый, рабочий), в котором панель инструментов не доступна.
\index{Seaside!deployment mode}
\index{Seaside!development mode}
Вы можете перевести Seaside в deployment режим используя страницу конфигурации или нажмите кнопку \button{Configure} на панели инструментов.
В любом случае, установите deployment режим на \emph{true}.
Заметье, изменения появятся только для следующих новых сессий.
...
Обычно это реализованно в методе \mthind{WAPresenter}
{renderContentOn:},
который получает в качестве аргумента экземпляр \clsind{WAHtmlCanvas}, умеющий, в свою очередь, создавать XHTML.
\index{Seaside!rendering}
...
\url{http://localhost:8080/seaside/config}, добавте новую точку входа назвав ее
<<hello>>, далее установите в качестве корневого компонента (root component) \ct{WAHelloWorld}.
Теперь откройте в обозревателе \url{http://localhost:8080/seaside/hello}.
Вот и все! Вы увидете веб-страницу как на \figref{WAHelloWorld}.}
...
нам нужно чтобы Seaside переводил в правильное состояние объект \ct{WACounter}.
...
Компонент разрешает возрат в предыдущее состояние реализацией метода \ct{states}
на стороне экземпляра.
Метод \ct{states} должен возвращать массив, содержащий все предыдущие состояния.
В нашем случае, объект \ct{WACounter} добавляет самого себя в таблицу предыдущих состояний,
возвращая \ct{Array with: self}.
...
Seaside требует регистрации вложенных компонентов реализацией метода \ct{children},
...
который является  \seeindex{canvas}{html canvas} html-полотном,
и где компоненты отображают себя.
По соглашению html-полотно называется \ct{html}.
...
Сообщение \ct{render: anyObject} может быть отправлено html-полотну,
...
Такой способ будет использоваться в multi-counter (смотрите \figref{WAMultiCounter}).
...
Полотно имеет определенное количество \emphind{кистей},
...
Корневой компонент \ct{SeasideDemo} просто прорисовывает вложенные компоненты,
...
которое прорисовывает на основе изменных свойств.}
...
с использованием каскадной таблицы стилей (\ind{CSS}).
...
\important{Не надо распологать программный код в одном методе.
Лучше разделить его на несколько вспомогательных
и назвать каждый метод по принципу \ct{render*On:}.
Все методы прорисовки входят в протокол \prot{rendering}.
...
Дале приведен код компонента \ct{SeasideFormDemo} представленный на
...
(Обратите внимание на \clsind{WAFileLibrary}, которая обеспечивает путь
...
\figref{democss} илюстрирует простую таблицу стилей,
...
для заговков второго уровня (\ct{h2}), таблиц (\ct{table}) и
...
Приложение \emphsubind{Seaside}{Sushi Store} илюстрирует многие из свойств
...
компонент Sushi store, экземплряр \clsind{WAStore}.
...
Обратие внимание, что когда транзакция закрыта,
...
проверте их. Ниже показан пример теста.}
...
Всем трем компонентам нужен будет доступ к общему экземляру стековой
...
Для корректного отображениия вложенных компонентов, Вам потребуется
...
клавиатуру как в нижерасположенным коде:
...
Попробуйте ввести несколько чисел, нажимая на цифровые кноки. Кнока

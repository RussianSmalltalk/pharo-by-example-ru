...
\item[Литералы.] Долларовый знак и последующий литерал обозначает экземпляр класса Character, например \ct{$a}.
...
\item[Строки.] Одиночные кавычки используются для обозначения экземпляра класса String, состоящего из последовательности букв и символов.
		Если нужно в строку (String) добавить одинарную кавычку, то ее нужно продублировать, например \ct{'G''day'}.
...
\item[Символы] также как строки (Strings), содержат последовательность букв.
		...
		В системе может быть только один символьный объект \ct{#hello}, а строк 'hello' может быть несколько.
		\seeindex{\#@{\textsf{\#}}}{literal symbol}
...
\item[Массивы создаваемые во время компилирования метода] можно определять при помощи \ct{#()}, где между скобками можно вводить литеральные объекты, разделяя их пробелами.
		...
		Например, \ct{#(27 (true false) abc)} состоит из трех элементов: целочисленное число 27, массива содержащего два булевых элемента и символа \ct{#abc}.
...
\item[Бинарные сообщения] --- операторы (\ct{+}\ct{-}\ct{/}\ct{-}\ct{>} и т.д.) посылаемые получателю и содержащие один аргумент.
	В \st! предложении \ct{3 + 4}, где получатель 3, бинарный селектор \ct{+} и аргумент 4.
...
\item[Каскадные сообщения.] Точка с запятой используется для посылки нескольких сообщений одному получателю.
	...
	Сначала посылается сообщение \ct{show: 'hello'} получателю Transcript.
...
Number, Character, String and Boolean подробно будут описаны в главе \charef{basic}

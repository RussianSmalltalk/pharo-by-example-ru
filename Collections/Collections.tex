...
\paragraph{Removing Elements.} Метод \mthind{OrderedCollection}{remove:} \ct{anObject} удаляет первое вхождение объекта из коллекции. Если коллекция не содержит такого объекта, выдаётся ошибка.
...
Класс \clsindmain{Bag} (Множество с повторяющимися элементами) практически тот же \ct{Set}, за исключением того, что допускает в себе наличие дубликатов.
\begin{code}{@TEST}
...
%  \item A key distinction is between \ct{SequenceableCollection}s, which maintain their elements in a given order, \ct{Dictionary} and its subclasses, which maintain key-to-value associations, and \ct{Set}s and \ct{Bag}s, which are unordered.
  \item Ключевые различия существуют между классами \ct{SequenceableCollection}, поддерживающим свои элементы в определённом порядке, \ct{Dictionary} и его подклассами, которые поддерживают ассоциации ключ-значение, а также \ct{Set} и \ct{Bag}, элементы которых не имеют установленного порядка.
%  \item You can convert most collections to another kind of collection by sending them the messages \ct{asArray}, \ct{asOrderedCollection} \etc.
  \item Вы можете конвертировать большинство коллекций в коллекции других типов, посылая им сообщения \ct{asArray}, \ct{asOrderedCollection} \etc.
%  \item To sort a collection, send it the message \ct{asSortedCollection}.
  \item Чтобы отсортировать коллекцию, пошлите ей сообщение \ct{asSortedCollection}.
%  \item Literal \ct{Array}s are created with the special syntax \ct{#( ... )}.  Dynamic \ct{Array}s are created with the syntax \ct|{ ... }|.
  \item Литеральные массивы создаются с помощью специального синтаксиса \ct{#( ... )}. Динамические массивы создаются с помощью синтаксиса \ct|{ ... }|.
%  \item A \ct{Dictionary} compares keys by equality. It is most useful when keys are instances of \ct{String}. An \ct{IdentityDictionary} instead uses object identity to compare keys.  It is more suitable when \ct{Symbol}s are used as keys, or when mapping object references to values.
  \item Класс \ct{Dictionary} проверяет ключи на равенство. Он удобен, если ключи являются экземплярами класса \ct{String}. Класс \ct{IdentityDictionary}, напротив, использует проверку ключей на идентичность. Его лучше использовать, если ключи являются экземплярами класса \ct{Symbol}, или при отображении ссылок на объекты на их значения.
...
%  \item The basic iteration message is \ct{do:}. It is useful for imperative code, such as modifying each element of a collection, or sending each element a message.
  \item Базовое сообщение для итераци -- \ct{do:}. Оно удобно для императивного кода, такого, как модификация каждого элемента коллекции, или посылка каждому элементу сообщения.
%  \item Instead of using \ct{do:}, it is more common to use \ct{collect:}, \ct{select:}, \ct{reject:}, \ct{includes:}, \ct{inject:into:} and other higher-level messages to process collections in a uniform way.
  \item Вместо использования \ct{do:}, часто лучше использовать \ct{collect:}, \ct{select:}, \ct{reject:}, \ct{includes:}, \ct{inject:into:} и другие более высокоуровневые сообщения для единообразной работы с коллекции.
...
%  \item If you override \ct{=}, remember to override \ct{hash} as well!
  \item Если вы переопределяете метод \ct{=}, не забудьте переопределить также и метод \ct{hash}!
\end{itemize}

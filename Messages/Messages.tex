...
\lct{[\emph{выражение}]} создаст \ugh{\ind{блок}-замыкание} (\ind{block} closure) (\ie некоторый объект) из \lct{\emph{выражения}}, который может быть выполнен произвольное число раз (в том числе ноль) в зависимости от контекста. Заметьте, что выражение может быть посылкой сообщения, переменной, литералом, присваиванием или блоком.
...
Поэтому условные ветви сообщений \ct{ifTrue:} or \ct{ifTrue:ifFalse:} принимают блоки. Следуя этому же принципу, и получатель и аргумент сообщения \ct{whileTrue:} требуют использования квадратных скобок, т.к. мы не знаем, сколько раз должен быть выполнен каждый из них.
...
Поэтому \lct{(\emph{выражениие})} будет выполнено ровно один раз.
...
\st позволяет, используя точку с запятой (\ct{;}), отправить несколько сообщений одному получателю. На жаргоне \st это называется \emphind{каскад} (cascade).

%\important{Expression Msg1 ; Msg2}
\important{Выражение сообщение1 ; сообщение2}
...
Фактически, получатель всех каскадных сообщений -- это получатель первого сообщения, входящего в каскад. В следующем примере \ugh{первое каскадное сообщение -- \ct{setX:setY}, т.к. за ним идёт каскад}. Получателем каскадного сообщения \ct{setX:setY:} является точка, создаваемая в результате вычисления \ct{Point new}, а \emph{не} \ct{Point}. Последующее сообщение \ct{isZero} посылается тому же получателю.
...
Они имеют форму \lct{получатель \textbf{селектор}}.
...
\item Бинарными называются сообщения, затрагивающие два объекта, получателя и какой-либо другой объект, \emph{а также} имеющие селектор, состоящий из одного или более следующих символов: \ct{+}, \ct{-}, \ct{*}, \ct{/}, \ct{|}, \texttt{\&}, \ct{=}, \ct{>}, \ct{<}, \texttt{\~} и \ct{@}.
...
Они имеют форму:
%\lct{receiver \textbf{selectorWordOne:} argumentOne \textbf{wordTwo:} argumentTwo}
\lct{получатель \textbf{первоеСловоСелектора:} первыйАргумент \textbf{второеСловоСелектора:} второйАргумент}
...
\item В \st традиционные арифметические операторы, такие как + и * имеют одинаковый приорите. \ct{+} и \ct{*} являются всего лишь бинарными сообщениями, поэтому \ct{*} не имеет приоритета перед \ct{+}. Используйте скобки, чтобы получить другой результат.
\end{itemize}

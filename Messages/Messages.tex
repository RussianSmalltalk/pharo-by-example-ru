...
Эта глава поможет вам разобраться в специальном синтаксисе для отправки сообщений.
...
В \st всё является отправкой сообщений, кроме элементов синтаксиса, описанных в \charef{syntax} (\ct+:= ^ . ; # () {} [ : | ]+).
...
В \st порядок в котором отправляются сообщения определяется видом сообщения. Таковых видов имеется три: \emphsubind{message}{унарные сообщения (unary)}, \emphsubind{message}{бинарные сообщения (binary)}, and \emphsubind{message}{сообщения с ключевыми словами (keyword)}. Унарные сообщения всегда отправляются первыми, затем идут бинарные сообщения, и, наконец, сообщения с ключевыми словами. Как и в большинстве языков, \ind{скобки} могут быть использованы для изменения порядка выполнения. Эти правила облегчают, насколько возможно, чтение \st кода, так, что в б'ольшую часть времени вы даже не будете о них вспоминать.
...
\begin{itemize}
  ...
  \item Комбинация сообщения и его получателя называется \emphsubind{message}{отправкой сообщения (message send)}, как показано на \figref{firstScriptMessage}.
\end{itemize}
...
\begin{minipage}{0.43\textwidth}
	...
	\caption{Выражение \ct{aMorph color: Color yellow}, включающее в себя отправку двух сообщений: \ct{Color yellow} и \ct{aMorph color: Color yellow}.\figlabel{ellipse}}
	\end{center}
\end{minipage}
\end{figure}
...
Мы также будем заключать каждое сообщение в овал, а также нумеровать сообщения, начиная с первого отправленного, чтобы помочь вам разобраться с порядком отправки сообщений.
...
На \figref{ellipse} показана отправка двух сообщений: \ct{Color yellow} и \ct{aMorph color: Color yellow}, поэтому там присутствуют два овала. Отправка сообщения \ct{Color yellow} выполняется первой, и соответствующий овал пронумерован единицей. Оба получателя: \ct{aMorph}, которому адресовано сообщение \ct{color: ...} и \ct{Color}, которому отправлено сообщение \ct{yellow}, выделены подчёркиванием.
...
Получатель может быть первым элементом сообщения, как, например, \ct{100} \ugh{в отправке сообщения} \ct{100 + 200}, или \ct{Color} в \ct{Color yellow}. Однако получатель может быть результатом другого сообщения. Например, в \ct{Pen new go: 100} получателем сообщения \ct{go: 100} является объект, возвращаемый в результате отправки сообщения \ct{Pen new}. Во всех этих случаях, сообщение отправляется объекту, называемому \emph{получателем (receiver)}, который, в свою очередь, может быть результатом отправки другого сообщения.

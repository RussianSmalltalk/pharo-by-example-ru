...
Переменные экземпляра доступны для всех методов класса и его потомков, где они были определены. Это означает, что в \st переменные экземпляра похожи на \emph{protected} переменные в C++ и Java. Но постарайтесь думать, что они приватные, в \st не принято напрямую обращаться к переменным экземпляра из потомка.

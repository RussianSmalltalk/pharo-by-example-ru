...
С технической точки зрения это означает, что \emphind{метаобъекты} (\emphind{metaobjects}) системы времени выполнения могут быть \ugh{представлены} (\emph{reified}) как обычные объекты, которые можно запрашивать и инспектировать.
...
\begin{figure}[ht]\centering
	...
	\caption{Представление и отражение.\figlabel{reflect}}
\end{figure}
...
\ind{Метрики} кода измеряют такие аспекты, как глубина иерархии наследования, количество прямых и непрямых подклассов, количество методов или переменных экземпляра для каждого класса, или количество локально определённых методов или переменных экземпляра.
...
Большинство сообщений, которые мы сейчас рассмотрим, реализованы в классе \ct{Behavior}, и поэтому могут обрабатываться всеми классами.
...
\item \mthind{Behavior}{whichSelectorsAccess:} возвращает список селекторов всех методов, которые осуществляют чтение или запись в переменную экземпляра с заданным именем;
...
\item \mthind{Behavior}{unreferencedInstanceVariables} возвращает список переменных экземпляра, которые не используются ни в классе получателя, ни в его подклассах.
\end{itemize}
...
Следующие сообщения, пожалуй, не нуждаются в дополнительных пояснениях (\ugh{Для англоязычного читателя, видимо. Может сделать пояснение по каждому из них?}):
...
Заметьте, что сообщения, реализованные, но никем не отправляемые, совсем не обязательно являются бесполезными, т.к. они могут посылаться неявно (например, с помощью \ct{perform:}).
...
Возможно, это не так широко известно, но множество предустановленных запросов реализовано методами из протокола \prot{browsing} класса \ct{SystemNavigation}.
...
\begin{figure}[ht]\centering
	...
	\caption{Поиск всех классов, реализующих \ct!\#ifTrue:!.\figlabel{implementors}}
\end{figure}
...
Особенно полезны методы \ct{browseAllSelect:} и \lct{browseMethodsWithSourceString:}. Вот два разных способа найти все методы в системе, производящие отправку сообщений суперклассу (первый способ более грубый; второй же более правильный, т.к. позволяет избежать некоторых ложных срабатываний):
...
На \figref{CompiledMethod} \ind{эксплорер} (\ind{explorer}) показывает структуру класса \clsind{Point}.
...
Селектор \ct{#*} указывает на декомпилированный \ind{байткод} (\ind{bytecode}) метода \ct!Point>>>*!.

\begin{figure}[ht]\centering
	\includegraphics[width=.5\linewidth]{CompiledMethod}
	%\caption{Explorer class \ct!Point! and the bytecode of its \ct!\#*! method.\figlabel{CompiledMethod}}
	\caption{Окно обозревателя с классом \ct!Point! и байткодом его метода \ct!\#*!.\figlabel{CompiledMethod}}
\end{figure}
...
На \figref{MethodsAsObjects} мы можем видеть, что классы и метаклассы имеют общий суперкласс \ct{Behavior}, который, среди ключевых методов классов, определяет метод \mthind{Behavior}{new}.
%Every class has a method dictionary, which maps method selectors to \ind{compiled methods}.
У каждого класса есть словарь методов, который отображает селекторы методов на \ind{скомпилированные методы} (\ind{compiled methods}).
...
На \figref{CompiledMethod} мы можем даже видеть, что эта связь хранится в объекте-ассоциации \ct{literal5}.
...
Например, чтобы узнать, какие новые методы (т.е. не переопределяющие методы суперкласса) определены в интересующем нас классе, мы можем перейти от класса к \ind{словарю методов} (\ind{method dictionary}) следующим образом:
...
Класс \ct{MethodReference} является облегчённым заместителем скомпилированного метода, используемым во множестве инструментов.
...
Чтобы получить такой объект-заместитель, можно воспользоваться методом \clsmthind{CompiledMethod}{methodReference}.
\begin{code}{@TEST}
...
\ugh{Неверный перевод заголовка!!!!}
...
Хотя класс \clsind{SystemNavigation} предоставляет несколько способов программно составлять запросы для исследования кода системы, существует лучший способ. \ind{Браузер рефакторинга} (\ind{Refactoring Browser}), интегрированный в \pharo, позволяет составлять подобные запросы как в интерактивном режиме, так и программно.
...
Предположим, нам нужно узнать, какие методы в иерархии класса \lct{Collection} посылают суперклассу сообщение, имя которого отличается от их собственного.
...
\begin{figure}[ht]\centering
	...
	\caption{Поиск методов, которые посылают суперклассу сообщение с именем, отличным от своего собственного.\figlabel{sendDifferentSuper}}
\end{figure}
...
Взгляните на протокол \prot{querying} класса \ct{RBProgramNode}, чтобы узнать, какую информацию хранят деревья разбора.
...
С другой стороны, \ind{отладчик}, очевидно, имеет доступ к этой информации: мы можем запросто исследовать контекст времени выполнения точно также, как и любой другой объект.
...
Всякий раз, когда выполяющийся метод ссылается на \ct{thisContext}, весь его контекст времени выполнения воплощается внутри образа как серия вложенных объектов класса \clsind{MethodContext}.
...
\begin{figure}[ht]\centering
	...
	\caption{Исследование псевдо-переменной \lct{thisContext}.\figlabel{exploringThisContext}}
\end{figure}
...
Если вы сейчас откроете определение класса исследуемого объекта (например, выполнив \ct{self browse} в нижней панели эксплорера), то увидите, что он является экземпляром класса \lct{MethodContext}, равно как и каждый отправитель сообщения в цепочке методов.
...
\ct{thisContext} не предназначен для использования в повседневной работе, но он крайне важен для реализации таких инструментов, как отлдачик, и для получения информации о стеке вызовов.
%You can evaluate the following expression to discover which methods make use of \ct{thisContext}:
Вы можете выполнить следующее выражение, чтобы найти методы, обращающиеся к \ct{thisContext}:
...
Как выясняется, одно из наиболее типичных применений \ct{thisContext} --- получение информации об отправителе сообщения.
...
По соглашению, в \st методы, которые посылают \ct{self subclassResponsibility}, считаются асбтрактными. Но каким образом \clsmthind{Object}{subclassResponsibility} выдаёт подробное сообщение об ошибке, указывая, какой абстрактный метод был вызван? Очень просто --- запрашивая у \ct{thisContext} информацию об отправителе.
...
К несчастью, это приводит к некоторым проблемам с методами, интенсивно используемыми системой.
...
Всё проясняется, стоит нам понять, что (i) метод \ct{OrderedCollection>>>add:} используется во многих частях системы, почему точка останова и срабатывает сразу после того, как мы приняли изменения; (ii) \emph{сам отладчик} посылает \ct{add:} экземпляру \ct{OrderedCollection}, не давая собственному окну открыться!
...
Это в точности то, что предлагает нам метод \clsmthind{Object}{haltIf:}.

%Suppose now that we only want to halt if \ct{add:} is sent from, say, the context of \ct{OrderedCollectionTest>>>testAdd}.
Предположим теперь, что мы хотим остановить выполнение, только если сообщение \ct{add:} было отправлено, скажем, из контекста метода \ct{OrderedCollectionTest>>>testAdd}.
...
Если такого метода не существует, он продолжает поиск вверх по иерархии классов, пока не достигнет класса \ct{Object}. Если для данного сообщения метод всё же не найден, объект \emph{отправляет самому себе} сообщение \ct{doesNotUnderstand:} с селектором исходного сообщения в качестве аргумента.
...
Оказывается, это подходящий способ реализации некоторых видов крайне динамического поведения. Объект, который не смог обработать сообщение, может, переопределив \ct{doesNotUnderstand:}, прибегнуть к альтернативной стратегии обработки этого сообщения.
...
Два очень распространённых приложения этой техники: (1) реализация \ind{облегчённых объектов-заместителей} (\ind{lightweight proxies}) объектов, и (2) динамически компилировать или загружать недостающий код.
...
Заметьте, что мы наследуемся от класса \ct{ProtoObject}, а не \ct{Object}, потому что мы не хотим, чтобы наш объект-заместитель наследовал более 400 методов (!) класса \ct{Object}.
...
Наш заместитель имеет две переменных экземпляра: \ct{subject} (субъект) -- объект который замещается, и \ct{count} -- счётчик перехваченных сообщений.
...
Мы создаём новый объект класса \ct{Point} (точка) и новый объект класса \ct{LoggingProxy}, и дальше, посылая сообщение \mthind{ProtoObject}{become:}, мы приказываем заместителю стать объектом-точкой.
...
Разгадка заключается в том, что сообщение \ct{class} в действительности никогда не посылается, а напрямую вычисляется виртуальной машиной.\footnote{Сообщение \ct{yourself} также никогда не посылается.
...
Наш объект-заместитель обокрали на целых два таких сообщения в методе \ct{rect:}:
\begin{code}{}
Point>>>rect: aPoint 
...
Хотя сообщения могут быть перехвачены объектами-заместителями, используя эту технику, вы должны помнить о её неотъемлемых ограничениях. В \secref{wrapper} мы рассмотрим другой, более общий подход к перехвату сообщений.
...
(1) Мы отправляем сообщение \ct{x} объекту \ct{myDA}, (2) в чьём классе производится поиск этого сообщения, который, (3) пробежав по иерархии наследования, заканчивается неудачей. (4) Это приводит к отправке объекту сообщения \ct{self doesNotUnderstand: #x}, (5) запускающего новый процесс поиска. В этот раз метод \ct{doesNotUnderstand:} тотчас находится в классе \ct{DynamicAccessors}, (6) который обращается к собственному классу с просьбой скомпилировать строку \ct{'x ^ x'}. Производится поиск метода \ct{compile}, который(8), наконец, находится в классе \ct{Behavior}, и тот (9-10) добавляет вновь скомпилированный метод в словарь методов класса \ct{DynamicAccessors}. Наконец, (11-13) исходное сообщение отправляется повторно, и на этот раз будет найдено.
...
Обратите внимание на использование в шаге (13) метода \clsmthind{Object}{perform:}, который используется для отправки сообщений, составляемых во время выполнения.
\begin{code}{@TEST}
...
Возможно, более неожиданным является то, что \emph{любой объект} может играть роль скомпилированного метода. Всё, что ему для этого нужно --- это отвечать на \ct{run:with:in:} и несколько других важных сообщений.
...
\dothis{Определите пустой класс \ct{Demo}. Выполните \ct{Demo new answer42} --- появилось обычное сообщение об ошибке <<Неопознанное сообщение>>.}
...
Если мы посмотрим на класс \clsind{ObjectsAsMethodsExample}, то обнаружим там следующие методы:
...
Если повнимательнее посмотреть на \ct{ObjectsAsMethodsExample}, можно заметить, что его суперкласс также реализует методы \ct{flushcache}, \ct{methodClass:} и \lct{selector:}, но все они пусты. Эти сообщения могут быть отправлены скомпилированным методам, поэтому они должны быть реализованы объектом, пытающимся играть их роль. (\ct{flushcache} --- наиболее важный из них; другие могут быть востребованы в зависимости от того, был ли метод установлен с помощью \clsmthind{Behavior}{addSelector:withMethod:} или напрямую через \clsmthind{MethodDictionary}{at:put:}.)
...
В оригинальной реализации\footnote{http://www.squeaksource.com/MethodWrappers.html} обёртка метода --- это экземпляр подкласса класса \ct{CompiledMethod}. Будучи установленной, обёртка метода может выполнять специфические действия до или после вызова оригинального метода.
...
В Pharo \ind{обёртки методов} (\ind{method wrappers}) могут быть реализованы гораздо проще: вместо того, чтобы наследоваться от \ct{CompiledMethod}, можно просто определить метод \ct{run:with:in:}. Фактически, уже существует облегчённая реализация объектов как обёрток методов\footnote{http://www.squeaksource.com/ObjectsAsMethodsWrap.html}, которая, однако, на момент написания этой книги не является частью стандартного дистрибутива Pharo.

%Nevertheless, the Pharo Test Runner uses precisely this technique to evaluate test coverage.
Тем не менее, \ugh{Pharo Test Runner} использует именно эту технику для определения покрытия тестами.
...
Метод \clsmthind{TestRunner}{collectCoverageFor:} ясно иллюстрирует алгоритм вычисления покрытия:
\begin{code}{}
...
а метод \ct{run:with:in:} просто обновляет переменную \ct{hasRun}, удаляет обёртку (после того, как покрытие было вычислено) и пересылает сообщение оригинальному методу:
\begin{code}{}
...
(Загляните в метод \clsmthind{ProtoObject}{withArgs:executeMethod:}, чтобы посмотреть, как можно вызывать метод, удалённый из своего словаря методов.)
...
\emphind{Прагма} (\emphind{pragma}) --- это аннотация, которая содержит некоторые сведения о программе, но не вовлечена в процесс её выполнения. Прагмы не оказывают прямого эффекта на функционирование метода, который они аннотируют.
...
\item Информация для компилятора: \indmain{прагмы} используются компилятором, чтобы поместить в метод вызов примитивной функции. Эта функция должна определяться виртуальной машиной или её дополнением.
...
Рассмотрим прагму \ct{<primitive: 73>}, содержащуюся в методе \ct{instVarAt:}. Селектором прагмы является \ct{primitive:}, а её аргументом --- \ugh{непосредственное значение} литерала \ct{73}.
...
Будучи экземплярами класса \clsind{Pragma}, прагмы являются полноценными объектами (first class objects). Скомпилированный метод отвечает на сообщение \mthind{CompiledMethod}{pragmas}, которое возвращает массив прагм.
...
Класс \ct{Pragma} определяет ещё несколько разновидностей метода \ct{allNamed:in:}, которые вы найдёте на стороне класса.
...
Прагма <<знает>> о том, в каком методе она определена (метод \ct{method}), имя этого метода (\ct{selector}), класс, к которому данный метод относится (\ct{methodClass}), колчество аргументов метода (\ct{numArgs}), а также то, какие литералы использовались в качестве аргументов прагмы (\ct{hasLiteral:} и \ct{hasLiteralSuchThat:}).
...
\item Инспектор использует метод \ct{instVarAt:} и аналогичные ему методы, чтобы запрашивать и модифицировать <<приватные>> переменные объектов.
...
\item Каждый класс в \st ссылается на экземпляр класса \ct{MethodDictionary}, который отображает селекторы на экземпляры класса \ct{CompiledMethod}. Скомпилированный метод, в свою очередь, содержит ссылку на класс, \ugh{замыкая тем самым круг}. 
...
\item Класс \ct{MethodReference} представляет собой облегчённый объект-заместитель скомпилированного метода, предоставляющий удобные вспомогательные методы. Используется многими \st-инструментами.
...
\item Псевдопеременная \ct{thisContext} является воплощением стека вызовов виртуальной машины. Она, в основном, используется отладчиком для динамического создания интерактивного представления стека, хотя также особенно полезна для динамического определения отправителя сообщения.
...
\item <<Умные>> точки останова можно установить, используя метод \ct{haltIf:}, принимающий селектор метода в качестве аргумента. \ct{haltIf:} останавливает выполнение только в том случае, если этот метод содержится в стеке предшествующих вызовов.
...
\item Будьте внимательны, некоторые сообщения, такие как \ct{class} и \ct{yourself} на самом деле никогда не посылаются, а напрямую выполняются виртуальной машиной. Другие, такие, как \ct{+}, \ct{-} и \ct{ifTrue:} также могут быть вычислены напрямую виртуальной машиной или встроены, в зависимости от получателя.
...
\item Для более тщательного перехвата сообщений используйте объекты как обёртки методов. Такой объект может быть установлен в словарь методов вместо скомпилированного метода. Такой объект должен реализовывать метод \ct{run:with:in:}, который посылается виртуальной машиной, когда она обнаруживает, что место скомпилированного метода в словаре методов занимает обычный объект. Эта техника используется в \ugh{SUnit Test Runner} для вычисления покрытия кода тестами.
\end{itemize}

...
С технической точки зрения это означает, что \emphind{метаобъекты} \ugh{времени выполнения} могут быть представлены как обычные объекты, информацию о которых можно запросить и проинспектировать.
%The metaobjects in \st are classes, metaclasses, method dictionaries, compiled methods, the run-time stack, and so on.
Метаобъектами в \st являются классы, метаклассы, словари методов, скомпилированные методы, стек \ugh{вызова методов} и т.д.
...
В \st можно вносить в представление метаобъектов изменения, которые будут \emph{отражаться} на \ugh{работающей системе} (см. \figref{reflect}). 
...
Чтобы быть рефлексивным, язык программирования должен поддерживать и \ind{интроспекцию} и \ind{посредничество}.
...
\emph{Структрурная рефлексия} заключается в исследовании и изменении структур рантайм системы, в то время как \emphind{поведенческая рефлексия} заключается в изменении способа интерпретации этих структур.

%In this chapter we will focus mainly on \ind{structural reflection}.
Эта глава фокусируется в основном на \ind{структурной рефлексии}.
...
\dothis{Теперь введите \ct!'hello'! вместо пустой строки и \emph{примените} изменения.}
...
\ugh{В \st атрибуты определяются классами по имени или по числовому индексу}.
...
\important{\emph{Предупреждение:} Хотя эти методы полезны при создании инструментария разработчика, использование их для разработки обычных приложений не самая лучшая идея, т.к. эти методы нарушают границу инкапсуляции объектов, тем самым делая код более сложным для понимания и сопровождения.}
...
И \ct!instVarAt:!, и \ct!instVarAt:put:! являются \ind{примитивными методами} (\ind{primitive methods}), т.е. реализованными через примитивные операции виртуальной машины \pharo.
...
\figref{allInstanceVariables} \ugh{показывает как отобразить значения} атрибутов экземпляра \ct!w! класса \ct!Workspace!.
...
Например, чтобы получить все экзепляры класса \ct!SketchMorph! чьё свойство \ct!owner! ссылается на \ugh{world morph \ie images currently displayed ??????}, выполните следующие выражение:
\begin{code}{}
...
\important{\emph{Caveat:} Хотя эти возможности особенно полезны при создании инструментов разработчика, они обычно не используются в создании обычных приложений.
...
Запрос у объекта его класса, или списка поддерживаемых сообщений -- типичные признаки плохого дизайна, т.к. это нарушает принцип инкапсуляции.
...
Инструменты разработки, однако, не являются обычными приложениями, т.к. их предметная область само программное обеспечение. Это даёт им право глубоко проникать в детали кода.
...
Особенно полезны методы \ct{browseAllSelect:} и \lct{browseMethodsWithSourceString:}. Вот два разных способа найти все методы в системе, производящие \ugh{супер-посылку :)} (первый способ более прямолинейный, но второй более правильный, т.к. не выдаёт неверных результатов):

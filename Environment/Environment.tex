...
\dothis{В качестве иллюстрации того, что вы можете делать с помощью \ind{инспектор}а введите \ct{TimeStamp now} в окне рабочей области, а затем вызовите с помощью \actclick меню и выберите \menu{inspect it}.}
...
Вы также можете нажать \short{i} для вызова меню \menu{\textbf{i}nspect it}.)
\clsindex{TimeStamp}
...
Появится окно вроде того, что изображено на \figref{inspectTimeNow1}.
...
Это инспектор, своего рода окно во внутренний мир выбранного объекта, в данном случае экземпляра класса \mbox{\ct{TimeStamp}},
...
который был создан, когда вы выполнили выражение \ct{TimeStamp now}.
...
Если вы выберите \menu{self} вверху левой панели, на правой отобразится printstring-представление объекта.
...
Горизонтальная панель внизу инспектора --- это маленькая рабочая область. Это может оказаться полезным, потому что в этом окошке псевдопеременная \ct{self} связана с текущим выбранным объектом на левой панели.
%So, if you \menu{inspect it} on
Поэтому, если вы выполните команду меню \menu{inspect it} на
\begin{code}{}
...
на рабочей панели, вы получите объект \clsind{Duration}, который представляет собой временной интервал между сегодняшней полночью и моментом времени в который вы выполнили \ct{TimeStamp now}, представленным в виде экземпляра класса \ct{TimeStamp}, который вы инспектируете.
...
Можете также попробовать вычислить \ct{TimeStamp now - self}, чтобы узнать, как долго вы читали этот раздел книги!
...
В дополнение к \ct{self}, все переменные экземпляра текущего объекта включены в область видимости панели рабочей области, так что вы можете использовать их в выражениях и даже присваивать им новые значения. Например, если вы выберете корневой объект на левой панели и выполните \ct{jdn  := jdn - 1} в панели рабочей области, вы увидите, что значение переменной экземпляра \ct{jdn} изменится, а значение выражения \ct{TimeStamp now - self} увеличится на один день.
...
\dothis{Выберите \menu{self} в левой панели инспектора, затем сделайте \actclick и выберите \menu{explore (I)}.}
%The \ind{explorer} window looks like \figref{exploreTimeStampNow}.
Окно \ind{исследователя} выглядит как на \figref{exploreTimeStampNow}.
...
Если вы кликните на маленьком треугольничке рядом с \ct{root}, содержимое окна изменится и станет таким, как на \figref{exploreTimeStampNow2}, где показаны переменные исследуемого объекта.
%Click on the triangle next to \ct{offset}, and you will see \emph{its} instance variables.  
Кликните на треугольничек рядом с \ct{offset} (смещение), и вы увидите \emph{его} переменные.
...
Псевдопеременная \ct{self} связана не с корневым объектом, а с выбранным в данный момент. В область видимости также попадают переменные экземпляра выбранного объекта.
...
\dothis{Выполните \ct{Object explore} в окне рабочей области}
%This is the object that represents the class \ct{Object} in \pharo.
Вы получите объект, который представляет класс \ct{Object}.
...
Обратите внимание, что вы можете переходить напрямую к объектам, представляющим словарь методов и даже скомпилированным методам этого класса (см. \figref{ExploreObject}).
...
\ind{Отладчик} --- это, вероятно, наиболее мощный инструмент в \pharo! Он используется не только для отладки, но также и для написания нового кода.
...
\dothis{Используя браузер, добавьте следующий метод в класс \ct{String}:}
...
\ct{'readme.txt' suffix} в рабочей области и выберите в меню \menu{print it (p)}.
...
Вот так сюрприз! Вместо того, чтобы напечатать ожидаемый ответ \ct{'txt'}, появилось предотладочное окно (\clsind{PreDebugWindow}), как показано на \figref{PreDebugWindow}.
...
\begin{figure}[btp]
	...
	\caption{Предотладочное окно (\ct{PreDebugWindow}) сообщает нам об ошибке.}
...
Предотладочное окно (\ct{PreDebugWindow}) имеет заголовок, сообщающий нам, что за ошибка произошла, и показывает \emphind{трассировку стека} (\emphind{stack trace}) сообщений, которые привели к ошибке.
...
Начиная снизу стека находится \ct{UndefinedObject>>>DoIt}, представляющий собой код, который был скомпилирован и запущен, когда мы выделили выражение \ct{'readme.txt' suffix} в рабочей области и попросили \pharo напечать его результат.
%This code, of course, sent the message \ct{suffix} to a \clsind{ByteString} object (\ct{'readme.txt'}). 
Этот код, разумеется, послал сообщение \ct{suffix} объекту класса \clsind{ByteString} (\ct{'readme.txt'}).
...
Это привело к выполнению метода \ct{suffix}, унаследованного от класса \ct{String}. Эта информация закодирована в следующей строке трассировки: \ct{ByteString(String)>>>suffix}.  
...
Внимательно рассмотрев стек, мы увидим, что метод \ct{suffix} посылает сообщение \ct{detect:}\ldots и в конечном счёте \ct{detect:ifNone} посылает \ct{errorNotFound}.

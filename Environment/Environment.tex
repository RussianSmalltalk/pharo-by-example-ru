...
Если вы взглянете на отправителей сообщения \ct{drawOn:} в \ct{AtomMorph>>>drawOn:}, вы увидите, что вызывается метод родительского класса (\subind{super}{send}).  Так что мы знаем, что выполняемый метод будет в родительском классе класса \ct{AtomMorph}. Что это за класс?  Нажмите ~ \menu{browse \go hierarchy implementors} и вы увидите, что это класс \ct{EllipseMorph}.
